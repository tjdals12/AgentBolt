...
export function renderNewAgentResult(result: NewItemResult, projectPath: string): void {
  const { catalogDir, packName, item, createdPaths } = result;
  const { name, description } = item;

  Banner.catalog.newAgent(name);
  console.log('');
  ...
  console.log('');
  console.log(chalk.dim('Optional settings — add to agent.json:'));
  ...
  console.log('');
  console.log(chalk.dim('Next, fill in the agent, then validate:'));
